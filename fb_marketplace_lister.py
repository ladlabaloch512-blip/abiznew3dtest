...
class MarketplaceLister:
    ...
    def create_listing(self, profile_dir, template_data, action="draft"):
        ...
        try:
            ...
            for img_path in template_data.get('images', []):
                print(f"📸 Uploading image via API: {img_path}")
            ...
            if not photo_ids and template_data.get('images', []):
                print("⚠️ No images were uploaded successfully, proceeding without images or aborting depending on requirements.")
    ...
    def _prepare_graphql_variables(self, template_data, photo_ids, profile_id, is_draft=True):
        ...
        # In FB's API, draft_type usually dictates if it's saved to draft or published.
        # "COMMERCE_SELL_OPTIONS" creates a draft, while omitting it or providing null/false often publishes,
        # but modern FB marketplace usually relies on the specific mutation to publish.
        # However, for useCometMarketplaceListingCreateMutation, setting draft_type=None and
        # draft_status="PUBLISHED" might be required. Let's provide None for draft_type to trigger publish.
            ...
            "input": {
                ...
                "data": {
                    "common": {
                        ...
                        "description": {"text": str(template_data.get('description') or '')},
    ...
    def draft_listing(self, driver, template_data, photo_ids, fb_dtsg, jazoest, lsd, profile_id):
        ...
        print("🚀 Saving listing as DRAFT via Browser XHR Injection...")
    ...
    def publish_listing(self, driver, template_data, photo_ids, fb_dtsg, jazoest, lsd, profile_id):
        ...
        print("🚀 PUBLISHING listing directly via Browser XHR Injection...")
        variables = self._prepare_graphql_variables(template_data, photo_ids, profile_id, is_draft=False)
        return self._execute_graphql_mutation(driver, variables, fb_dtsg, jazoest, lsd, profile_id)
# --- Testing the Module ---
if __name__ == "__main__":
    lister = MarketplaceLister()
    # Example dummy image, you can change this to a valid path.
    test_image_path = "dummy.jpg"
    # Just creating a dummy image file for testing purposes so it doesn't crash on open()
    try:
    ...
    dummy_template = {
        ...
        "price": 5, # Will be in USD as configured
        "category": "Household",
        "condition": "Used - Good",
        "description": "This is an automated test listing. Please ignore.",
        "location": "New York, NY",
        "door_meetup": True
    }
    # Action can be 'draft' or 'publish'
    lister.create_listing("test_profile", dummy_template, action="draft")
